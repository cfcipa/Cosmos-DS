...
import { AuiAssistantProvider, AuiSelectionContextProvider, AuiThread, useAuiAssistant } from '../../src/ai/aui';
import { AuiDemoRuntime } from '../ui/AuiDemoRuntime';
import { ElementPage, PropRow, PropToggle } from '../ui/Playground';
import { facturasModel, facturasSelection } from '../ui/sinco/facturas';
import { SelectionBar } from '../ui/sinco/parts';
      ...
      <AuiAssistantProvider>
        <AuiSelectionContextProvider selection={selection}>
          <Box sx={{ position: 'relative', height: '100%', p: 2, boxSizing: 'border-box' }}>
            <Box sx={(t) => ({ height: t.spacing(4.75) })}>{n ? <SelectionBar count={n} /> : null}</Box>
...
export function AuiAskAiActionCard() {
  return <Box sx={{ display: 'flex', justifyContent: 'center' }}><SelectionBar count={3} /></Box>;
}
